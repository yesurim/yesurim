...
**RAG Baseline 설계**와 **Generation 고도화 · Prompt Engineering**을 담당했습니다.

- **근거 기반 답변 (Faithfulness 0.87)** — 검색된 문서를 출처와 함께 제시하도록 생성 단계를 설계해 환각을 억제하고, 답변마다 판단 근거를 확인할 수 있게 함
- **하이브리드 검색으로 근거 문서 품질 개선** — FAISS(벡터) + BM25(키워드)를 결합하고 리랭킹을 적용해, 생성 단계로 넘어가는 문서의 **Hit@3를 0.46 → 0.78로 향상** (제도명·자격 조건처럼 키워드 일치가 중요한 항목 보완)
- **프롬프트 고도화** — GPT-4.1 Prompting Guide를 참고해 역할 부여·출력 형식·금지 규칙·Few-shot을 단계적으로 적용하고, Answer Correctness / Faithfulness 기준으로 비교·실험
- **응답 흐름 설계** — `질문 재작성 → 의도 분류 → 하이브리드 검색 → 리랭크 → 답변 생성 → 대화 요약`
- **멀티턴 & 예외 처리** — 직전 대화와 요약으로 조건을 유지해 연속 상담 지원, 복지와 무관하거나 부적절한 입력은 검색 전에 차단해 별도 안내
- **데이터 구조화** — 복지 정책 데이터를 수집·정제해 MySQL에 저장하고, 문서 임베딩 후 FAISS 인덱스 구축
